...
app.controller('sanphamAdmin-controller', function($scope, $http, $window, $sce) {
	...
	$scope.laymaHD = function(item) {
		var maHoaDon = item.maHoaDon;
		$scope.selectedMaHoaDon2 = maHoaDon;
	}

	$scope.capNhatTrangThaiTable = function(trangThaiHoaDon, ghichu) {
		Swal.fire({
			title: "Bạn chắc chắn không?",
			text: "Bạn sẽ không thể hoàn tác điều này!",
			icon: "warning",
			showCancelButton: true,
			confirmButtonColor: "#28a745", // Màu nút đồng ý
			cancelButtonColor: "#dc3545", // Màu nút hủy
			confirmButtonText: "Đồng ý",
			cancelButtonText: "Hủy"
		}).then(function(result) {
			if (result.value) {
				// Người dùng đã chọn "Đồng ý", thực hiện cuộc gọi API
				$http.put('/rest/' + $scope.selectedMaHoaDon2 + '/cap-nhat-trang-thai/' + trangThaiHoaDon, ghichu)
					.then(function(response) {
						...
						// Tìm modal hiện tại và tắt nó
						var modalElement = angular.element('#huydonhang');
						modalElement.modal('hide');
					})
					.catch(function(error) {
						// Xử lý lỗi nếu có
						console.error("Lỗi khi cập nhật hóa đơn:", error);
						Swal.fire("Lỗi", "Bạn phải ghi lý do hủy đơn hàng.", "error");
					});
			} else if (result.dismiss === Swal.DismissReason.cancel) {
				// Người dùng đã chọn "Hủy", không cần thực hiện gì cả
				Swal.fire("Hủy", "Bạn đã hủy cập nhật trạng thái hóa đơn.", "info");
			}
		});
	};
	
		$scope.capNhatTrangThaiTable2 = function(item, trangThaiHoaDon) {
		Swal.fire({
			title: "Bạn chắc chắn không?",
			text: "Bạn sẽ không thể hoàn tác điều này!",
			icon: "warning",
			...
			if (result.value) {
				// Người dùng đã chọn "Đồng ý", thực hiện cuộc gọi API
				$http.put('/rest/' + item.maHoaDon + '/cap-nhat-trang-thai2/' + trangThaiHoaDon)
					.then(function(response) {
						// Cập nhật thành công, hiển thị thông báo thành công
						Swal.fire("Thành công!", "Cập nhật trạng thái thành công!", "success");
					...
					.catch(function(error) {
						// Xử lý lỗi nếu có
						console.error("Lỗi khi cập nhật hóa đơn:", error);
						Swal.fire("Lỗi", "Cập nhật hóa đơn lỗi.", "error");
					});
			} else if (result.dismiss === Swal.DismissReason.cancel) {
				// Người dùng đã chọn "Hủy", không cần thực hiện gì cả
				Swal.fire("Hủy", "Bạn đã hủy cập nhật trạng thái hóa đơn.", "info");
	...
	$scope.capNhatTrangThaiPN = function(item) {
		var ma = item.maPhieuNhap;
		console.log(ma);

		swal
			.fire({
				title: "Xác Nhận Đơn Hàng",
				text: "Xác Nhận Đơn Hàng Đã Nhập Thành Công!",
				type: "info",
				showCancelButton: true,
				confirmButtonText: "Đã Nhập Hàng!",
				cancelButtonText: "Chưa Nhập Hàng!",
				reverseButtons: true,
			})
			.then(function(t) {
				if (t.value) {
					// Người dùng đã chọn "Đã Nhập Hàng", thực hiện cuộc gọi API
					$http.put('/update/phieuhang/trangThai/' + ma)
						.then(function(response) {
							swal.fire("Đã Nhập Hàng!", "Sản Phẩm Bạn Vẫn Chưa Nhập.", "success");
							$scope.capnhat();
						})
						.catch(function(error) {
							console.error("Lỗi khi cập nhật hóa đơn:", error);
					...
					// Người dùng đã chọn "Chưa Nhập Hàng"
					swal.fire("Chưa Nhập Hàng", "Bạn vẫn chưa nhập hàng:)", "error");
				}
			});
	};


	$scope.layma = function(item) {
		$scope.maPhieu = item.maPhieuNhap;

	};

	$scope.capNhatTrangThaiPN2 = function() {
		swal
			.fire({
				title: "Bạn có chắc không?",
				text: "Bạn sẽ không thể thay đổi được!",
				type: "warning",
				showCancelButton: true,
				confirmButtonText: "Đồng ý!",
			...
			.then(function(t) {
				if (t.value) {
					// Người dùng đã chọn "Đồng ý", thực hiện cuộc gọi API
					$http.put('/update/phieuhang/trangthai/' + $scope.maPhieu, $scope.ghichu)
						.then(function(response) {
							swal.fire("Xóa!", "Xóa thành công.", "success");
							$scope.capnhat();
							// Tìm modal hiện tại và tắt nó
							var modalElement = angular.element('#lydohuy');
							modalElement.modal('hide');
						})
						.catch(function(error) {
							console.error("Lỗi khi cập nhật hóa đơn:", error);
							Swal.fire({
								icon: "error",
								title: "Lỗi...",
								text: "Chưa chọn lý do hủy phiếu!",
								footer: "<a href>Vui lòng điền lý do</a>",
							});
						});
				} else if (t.dismiss === Swal.DismissReason.cancel) {
					// Người dùng đã chọn "Không"					
					swal.fire("Hủy", "Bạn đã không xóa :)", "error");
					// Tìm modal hiện tại và tắt nó
					var modalElement = angular.element('#lydohuy');
					modalElement.modal('hide');
				}
